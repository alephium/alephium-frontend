...
class CliqueClient {
  ...
  async getBalance(address) {
    ...
    return this.clients[group].getBalance(address);
  }

  async transfer(fromAddress, fromType, fromPrivateKey, toAddress, toType, value) {
    const group = await this.getGroupIndex(fromAddress);
    return this.clients[group].transfer(fromAddress, fromType, fromPrivateKey, toAddress, toType, value);
  }
}
