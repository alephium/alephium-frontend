...
class StoredState {
  constructor(seed, numberOfAddresses, activeAddressIndex, mnemonic) {
...
class Wallet {
  constructor(seed, address, publicKey, privateKey, mnemonic) {
    this.seed = seed;
  ...
  encrypt(password) {
    ...
    const storedState = new StoredState(this.seed, 1, 0, this.mnemonic)
...
function fromMnemonic(mnemonic, networkType) {
  ...
  return fromSeed(seed, mnemonic, networkType);
}

function fromSeed(seed, mnemonic, networkType) {
...
function walletGenerate(networkType) {
  ...
  return fromMnemonic(mnemonic, networkType);
}
...
async function walletOpen(password, data, networkType) {
  ...
  return fromSeed(Buffer.from(config.seed,'hex'), config.mnemonic, networkType)
}
