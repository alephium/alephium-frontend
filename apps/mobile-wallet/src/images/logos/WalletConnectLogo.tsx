...
const ASPECT_RATIO = 119 / 192

const WalletConnectLogo = ({ width = 192, height, color = '#fff' }: WalletConnectLogoProps) => {
  const resolvedHeight = height ?? Math.round(width * ASPECT_RATIO)

  return (
    <Canvas style={{ width, height: resolvedHeight }}>
    <FitBox src={rect(0, 0, 192, 119)} dst={rect(0, 0, width, resolvedHeight)}>
      <Path
        color={color}
