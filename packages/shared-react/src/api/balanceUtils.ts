...
export const getRefillMissingBalancesChainedTxParams = async ({
  ...
  const signerMissingBalances = await getSignerMissingBalances(expectedBalances, signerAddress, networkId, isNodeOnline)
  const remainingAddresses = addressesWithGroup.filter(({ hash }) => hash !== signerAddress)
  const address = await getAddressWithEnoughBalances(signerMissingBalances, remainingAddresses, networkId, isNodeOnline)
...
const getSignerMissingBalances = async (
  ...
  const { balances: signerBalances } = await queryClient.fetchQuery(
    addressBalancesQuery({ addressHash: signerAddress, networkId, isNodeOnline })
  )
...
const getAddressWithEnoughBalances = async (
  ...
  if (missingBalances.size > 0) {
    ...
    for (const address of addresses) {
      const { balances: addressBalances } = await queryClient.fetchQuery(
        addressBalancesQuery({ addressHash: address.hash, networkId, isNodeOnline })
      )
